 ...
 * Ramp configuration
 *
 * Safety constraint: current must not change faster than 0.1 mA/s to avoid
 * patient discomfort. At a 2 mA full-scale output:
 *
 *   t_min = 2 mA / 0.1 mA/s = 20 s minimum ramp duration
 *
 * RAMP_DURATION_MS is set to exactly this minimum. Do not reduce it without
 * re-evaluating the patient safety constraint.
 *
 * At 10 ms per step over 20000 ms this yields 2000 steps, each advancing
 * amplitude by 1/2000 = 0.05%.
 * -------------------------------------------------------------------------- */
#define MAX_CURRENT_MA          2.1f
#define MAX_RAMP_RATE_MA_PER_S  0.1f
#define RAMP_DURATION_MS        ((uint32_t)((MAX_CURRENT_MA / MAX_RAMP_RATE_MA_PER_S) * 1000.0f))
...
static void device_run(void)
{
    printf("\r\n========================================\r\n");
    printf("  Device ready.\r\n");
    printf("  Press TOGGLE (GP%u) to start.\r\n", PIN_TOGGLE_BTN);
    printf("  Press TOGGLE (GP%u) again to ramp down.\r\n", PIN_TOGGLE_BTN);
    printf("========================================\r\n\r\n");

    ButtonState toggle_btn = { PIN_TOGGLE_BTN, 0u, false, false };
    ...
    while (true) {
        ...
        bool     toggle_pressed = button_poll(&toggle_btn);
        uint64_t now          = time_us_64();
