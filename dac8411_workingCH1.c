...
void dac8411_init(void)
{
    ...
    gpio_init(DAC_PIN_SYNC);
    gpio_set_dir(DAC_PIN_SYNC, GPIO_OUT);
    gpio_put(DAC_PIN_SYNC, 1);  /* Deasserted - idles high */
}
 ...
 * -------------------------------------------------------------------------- */
void dac8411_write(uint16_t code)
{
    uint8_t frame[3];
    ...
    gpio_put(DAC_PIN_SYNC, 0);
    spi_write_blocking(DAC_SPI_PORT, frame, 3);
    gpio_put(DAC_PIN_SYNC, 1);
}